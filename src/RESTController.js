    ...
    return installationIdPromise
        ...
        if (options && typeof options.sessionToken === "string") {
          return Promise.resolve({
            ...
            sessionTwoFactorToken:
              typeof options.sessionTwoFactorToken === "string"
                ? options.sessionTwoFactorToken
                : ""
          });
          ...
          return userController.currentUserAsync().then(user => {
            if (user) {
              return Promise.resolve({
                ...
                sessionTwoFactorToken: user.getSessionTwoFactorToken()
              });
            }
      ...
      .then(token => {
        if (token && token.sessionToken) {
          payload._SessionToken = token.sessionToken;
        }

        if (token && token.sessionTwoFactorToken) {
          payload._SessionTwoFactorToken = token.sessionTwoFactorToken;
        }
